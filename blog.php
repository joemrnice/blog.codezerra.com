...
$postsPerPage = (int)($settings['posts_per_page'] ?? 9);
...
if ($categorySlug) {
    ...
    $stmt = $db->prepare("
        ...
        LIMIT ? OFFSET ?
    ");
    $stmt->execute([$categorySlug, $postsPerPage, $offset]);
    $posts = $stmt->fetchAll();
    ...
    $stmt = $db->prepare("
        ...
        LIMIT ? OFFSET ?
    ");
    $stmt->execute([$tagSlug, $postsPerPage, $offset]);
    $posts = $stmt->fetchAll();
    ...
    $countStmt = $db->prepare("
        ...
        WHERE p.status = 'published' AND t.slug = ?
    ");
    $countStmt->execute([$tagSlug]);
    $totalPosts = $countStmt->fetch()['count'];
    
    $tag = $db->prepare("SELECT * FROM tags WHERE slug = ?");
    $tag->execute([$tagSlug]);
    $tagData = $tag->fetch();
    $pageTitle = 'Tag: ' . ($tagData['name'] ?? $tagSlug);
} else {
    $posts = getPublishedPosts($postsPerPage, $offset);
    $totalPosts = getPostCount('published');
    $pageTitle = 'All Posts';
}
...
// Get popular tags (all tags for now)
...
$pagination = getPagination($page, $totalPosts, $postsPerPage, $baseUrl);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?php echo escape($pageTitle); ?> - <?php echo escape($siteTitle); ?></title>
    <meta name="description" content="Browse our blog posts on web development, programming, and technology">
...
<body class="bg-gray-50">
    ...
    <section class="bg-gradient-to-r from-blue-600 to-purple-600 text-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-4xl font-bold"><?php echo escape($pageTitle); ?></h1>
        </div>
    </section>
            ...
            <div class="lg:col-span-3">
                ...
                <?php if (empty($posts)): ?>
                    <div class="bg-white rounded-lg shadow-md p-12 text-center">
                        ...
                        <a href="/blog.php" class="text-blue-600 hover:text-blue-800 mt-4 inline-block">View all posts</a>
                    </div>
                <?php endif; ?>
                    ...
                    <div class="mt-12 flex justify-center">
                        <nav class="flex items-center space-x-2">
                            <?php if ($pagination['has_prev']): ?>
                                <a href="<?php echo escape($pagination['prev_url']); ?>" 
                                   class="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-100">
                                    Previous
                                </a>
                            <?php endif; ?>
                            
                            <?php foreach ($pagination['pages'] as $pageData): ?>
                                <?php if ($pageData['is_current']): ?>
                                    <span class="px-4 py-2 bg-blue-600 text-white rounded-md">
                                        <?php echo $pageData['number']; ?>
                                    </span>
                                <?php else: ?>
                                    <a href="<?php echo escape($pageData['url']); ?>" 
                                       class="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-100">
                                        <?php echo $pageData['number']; ?>
                                    </a>
                                <?php endif; ?>
                            ...
                            <?php if ($pagination['has_next']): ?>
                                <a href="<?php echo escape($pagination['next_url']); ?>" 
                                   class="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-100">
                                    Next
                                </a>
                            <?php endif; ?>
            ...
            <aside class="lg:col-span-1">
                    ...
                    <form action="/search.php" method="get">
                        <div class="flex">
                            <input type="text" 
                                   ...
                                   class="flex-1 px-4 py-2 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-blue-600">
                            <button type="submit" 
                ...
                <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                    ...
                    <ul class="space-y-2">
                        <?php foreach ($categories as $cat): ?>
                            <li>
                                <a href="<?php echo escape(getCategoryUrl($cat['slug'])); ?>" 
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <?php foreach (array_slice($popularTags, 0, 10) as $tag): ?>
                            <a href="<?php echo escape(getTagUrl($tag['slug'])); ?>" 
                               class="px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-sm hover:bg-blue-600 hover:text-white transition-colors">
